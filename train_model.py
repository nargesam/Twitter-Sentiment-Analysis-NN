...
tf.flags.DEFINE_integer("l2_reg_lambda", 0, "L2 regularizaion lambda (default: 0)")
...
# LSTM-specific
tf.flags.DEFINE_integer("lstm_hidden_size", 256, "Size of the hidden LSTM"
                                                 " layer.")
...
tf.flags.DEFINE_boolean("lstm", True,
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        if FLAGS.lstm:
            print("\nUsing LSTM.")
            print(type(FLAGS.lstm_hidden_layers), type(FLAGS.lstm_hidden_size), type(embedding_dim))
        ...
        with open(meta_fname, 'w') as meta_file:
            # future : Add whatever additional information is necessary.
        ...
        # Train Summaries
        # train_summary_op = tf.summary.merge([tf.summary.merge_all(loss_summary), tf.summary.merge_all(acc_summary)],collections='merged')
        train_summary_op = tf.summary.merge_all()
        # train_summary_op = tf.summary.merge_all([loss_summary, acc_summary])
